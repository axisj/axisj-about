...
How long to wait before emitting events in succession for the same filepath and status. For example if your `Gruntfile-for-samples.js` file was `changed`, a `changed` event will only fire again after the given milliseconds.
...
By default, if `Gruntfile-for-samples.js` is being watched, then changes to it will trigger the watch task to restart, and reload the `Gruntfile-for-samples.js` changes.
When `reload` is set to `true`, changes to *any* of the watched files will trigger the watch task to restart.
This is especially useful if your `Gruntfile-for-samples.js` is dependent on other files.

```js
watch: {
  configFiles: {
    files: [ 'Gruntfile-for-samples.js', 'config/*.js' ],
    options: {
      reload: true
    }
  }
...
grunt.initConfig({
  watch: {
    gruntfile: {
      files: 'Gruntfile-for-samples.js',
